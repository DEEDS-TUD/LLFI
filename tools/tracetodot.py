...
def executeTraceDiff():
    ...
    for file in os.listdir(currentpath):
        if file.endswith(".txt") and file.startswith("llfi.stat.trace."):
            cmd = tempScriptdir + "/tracediff " + tempgoldenTraceFilePath + " " + file + " > " + temptraceOutputFolder + \
                "/TraceDiffReportFile" + file[file.find("llfi.stat.trace") + len("llfi.stat.trace"):]
...
def generateDotFile():
    ...
    for file in os.listdir(traceOutputFolder):
        if file.startswith("TraceDiffReportFile"):
            # Parse the name
            name = file[file.find("TraceDiffReportFile") +
                        len("TraceDiffReportFile"):]
            name = name.replace("txt", "dot")
            cmd = tempScriptdir + "/traceontograph " + temptraceOutputFolder + "/" + file + \
                " " + tempgoldenTraceDotFile + " > " + temptraceOutputFolder + "/TraceGraph" + name
            p = subprocess.call(cmd, shell=True, stderr=log_file)
