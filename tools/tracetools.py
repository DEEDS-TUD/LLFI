...
class diffBlock:
    def __init__(self, lines):
        ...
        origHeader, newHeader = lines[0].replace(
                ...
                if g.ID == f.ID:
                    if instance.type != 1:
                        ...
                        instance = diffInstance(
                            1, findAdjustedPosition(
                                origStart, goldenRemovedCount), findAdjustedPosition(
                                newStart, faultyRemovedCount), i)
                    instance.add("Data Diff: ID: " +
                                 str(g.ID) +
                                 " OPCode: " +
                                 str(g.OPCode) +
                                 " Value: " +
...
class ctrlDiffBlock(diffBlock):
    def getRange(self):
        ...
        debug(str(self.origStart) + " " + str(self.origLength) +
              " " + str(self.newStart) + " " + str(self.newLength))
            ...
            if g and f:
                if instance.type != 2:
                    if (instance.summary is not None):
                        instanceList.append(
                            instance.summary(
                                self.preDiff, self.postDiff))
                    instance = diffInstance(2, origStart, newStart, i)
                instance.add("Ctrl Diff: ID: " +
                             str(g[1:]) + " \\ " + str(f[1:]))
                instance.incOrigLength()
                instance.incNewLength()
            if g and not f:
...
# The hack might not always work. Jiesheng
        ...
        if ctrldiff:
            ...
            while (i < len(ctrldiff)):
                if "@@ " in ctrldiff[i]:
                    if start is not None:
                        debug(
                            "Calling ctrlDiffBlock constructor " +
                            str(start) +
                            " " +
                            str(length))
            ...
            for block in self.blocks:
                goldenLines, faultyLines = removeRangeFromLines(
                    goldenLines, faultyLines, block.getRange(), self.startPoint)
                debug("Golden Lines:\n" + "\n".join(goldenLines))
                debug("Faulty Lines:\n" + "\n".join(faultyLines))
        ...
        if datadiff:
            ...
            while (i < len(datadiff)):
                if "@@ " in datadiff[i]:
                    if start is not None:
                        self.blocks.append(
                            diffBlock(datadiff[start:start + length]))
                        length = 1
                    start = i
                else:
                    length += 1
    ...
    def printSummary(self):
        ...
        self.blocks.sort(
            key=lambda x: (
                x.getSummary(
                    self.startPoint)).split("\n")[1].replace(
                '\\',
                ' ').split()[3])

        for block in self.blocks:
            if block.preDiff is None:
...
class faultReport:
    ...
    def report(self):
        ...
        lines.append(header)
        faultline = "ID: " + str(self.faultID) + \
            " OPCode: " + str(self.faultOPCode)
            ...
            if "Diff@" in self.diffs[i] and "Pre  Diff" in self.diffs[i + 1]:
                ...
                if csplit[5] != "None":
                    ...
                    if (i + 3 < len(self.diffs)):
                        affectedEdges.add(
                            (edgeEnd, int(self.diffs[i + 3].split()[5])))
                else:
                    ...
                    while d < len(self.diffs):
                        if "Post Diff" in self.diffs[d]:
                            ...
                            # If we found a new ctrl diff block before finding
                            # a post diff,
                            d = len(self.diffs)
                            # exit the loop early
                        d += 1
